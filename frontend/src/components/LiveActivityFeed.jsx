...
const getRelativeTime = (timestamp, t) => {
  ...
  if (isNaN(diffSec) || diffSec < 45) return t('justNow') || 'Just now';
  if (diffMin < 60) return `${diffMin}${t('mAgo') || 'm ago'}`;
  if (diffHr < 24) return `${diffHr}${t('hAgo') || 'h ago'}`;
  return `${diffDay}${t('dAgo') || 'd ago'}`;
};
    ...
    <div className="garden-card animate-fade-in" style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.65rem' }}>
          ...
          <div>
            <h4 style={{ fontSize: '1.15rem', color: '#FFFFFF', fontWeight: 800, margin: 0, fontFamily: 'var(--font-heading)' }}>
              {t('liveActivityTitle').toUpperCase()}
            </h4>
            <div style={{ fontSize: '0.78rem', color: '#9198A1', fontWeight: 600 }}>
              {t('liveActivitySub').toUpperCase()}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.4rem', fontSize: '0.75rem', color: '#34D399', fontWeight: 800, fontFamily: 'var(--font-esports)' }}>
          ...
          <span>{t('liveSync').toUpperCase()}</span>
        </div>
      </div>
      ...
      {loading ? (
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.65rem' }}>
          {activities.map((act) => {
            ...
            const translatedAction = rawAction.includes('completed') 
              ? rawAction.replace('completed', t('completedAction') || t('completed'))
              : rawAction;
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                  ...
                  <div>
                    ...
                    <span style={{ color: '#9198A1' }}>{translatedAction}</span>
                    {act.score && (
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.3rem', color: '#38BDF8', fontSize: '0.75rem', fontWeight: 800, fontFamily: 'var(--font-esports)' }}>
                  <Clock size={12} />
                  <span>{getRelativeTime(act.completedAt, t)}</span>
                </div>
              </div>
            );
          })}

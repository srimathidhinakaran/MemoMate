...
const MyDayPage = () => {
  ...
  const mainFamilyVisitor = familyMembers?.[0]?.name || 'Meena';
  const medicationItem = reminders.find(r => r.category === 'medicine')?.detail || 'Blood Pressure Medication';

  const storyText = `Today is a peaceful day. Your ${familyMembers?.[0]?.relation || 'daughter'} ${mainFamilyVisitor} will visit you in the evening for tea and garden walk. Your ${medicationItem} is scheduled after lunch. Keep hydrated throughout the day!`;

  const question1 = {
    text: `Who is visiting you this evening?`,
    options: [mainFamilyVisitor, 'Neighbor', 'Doctor'],
    correct: mainFamilyVisitor
  };
  ...
  const handleCheckAnswers = () => {
    let score = 0;
    if (selectedAnswer1 === question1.correct) score += 50;
